...
Adafruit_MQTT_Publish photo = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/smart_cont_ultra");
Adafruit_MQTT_Publish photocell = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/smart_container");
...
void loop() {
 ...
 int cont=digitalRead(D2);
Serial.println(cont);
photocell.publish(cont);

 if(distance1<=10&&distance1>=2)
 {
photo.publish(int(distance1));
delay(100);
 }
